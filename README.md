...
** todo **
- **Igterator.time_travel =** ( dt = 0;  amount_nodes * dims * e/node (=self.pattern) * schwellwert (=argmax((sum(prev_time_feature_store) / dims (=time step (ts) set) -> sum(item.nodes[item.index:100].e)) / dt) ))
